  ...
  val filesRoute =
    ...
    (path("upload") & extractLog) { log =>
      ...
      entity(as[Multipart.FormData]) { formdata =>
        ...
        val writeOperationFuture = partsSource.runWith(filePartsSink) // creats akka stream graph
        onComplete(writeOperationFuture) {
          // Post response to client whether successfully uploaded file or not
          case Success(_) => complete("Jay bahuchar Mataji File uploaded.")
