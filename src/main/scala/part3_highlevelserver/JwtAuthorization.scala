...
/**
  ...
  */
import spray.json._
object SecurityDoman extends DefaultJsonProtocol {
    ...
    post {
      entity(as[LoginRequest]) {
        case LoginRequest(username, password)
          ...
          respondWithHeader(RawHeader("Access-Token", token)) { // when you want to respond with header
        ...
        case Some(token) =>
          if (isTokenValid(token)) {
            ...
            complete(
              HttpResponse(
                status = StatusCodes.Unauthorized,
                entity = "Token is invalid or has been " +
                  "tempered"
              )
            )
          }
